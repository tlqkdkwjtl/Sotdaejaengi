...
//
// 저장 방식:
// - localStorage에 저장 (키: 'newsArchive_1', 'newsArchive_2', ...)
// - window 객체에도 저장 (즉시 접근 가능)
// - 5일마다 새 파일 번호 사용 (1일차-5일차: 파일1, 6일차-10일차: 파일2, ...)
//
// 호출 위치:
// - js/Sotdaejaengi.js의 Game.endOperatorActivity()에서 날짜 변경 시 호출

/**
 * 뉴스 저장 (매일 호출)
 * @param {Object} gameState - 게임 상태 객체 (day, news 배열 포함)
 * @description 현재 날짜의 뉴스를 localStorage에 저장합니다.
 *              5일마다 새 파일 번호를 사용하여 파일이 너무 커지지 않도록 합니다.
 */
function saveDailyNews(gameState) {
    const currentDay = gameState.day;  // 현재 날짜 (1부터 시작)
